 ...
 */
function rotationToTopAndLeftCoordinates(rx, ry) {
  ...
  render() {
    for(let _x=0; _x<BALL_WIDTH; _x++) {
      for(let _y=0; _y<BALL_WIDTH; _y++) {
        ...
        for(let rayX=0; rayX<RAY_CASTING_RESOLUTION; rayX++) {
          for(let rayY=0; rayY<RAY_CASTING_RESOLUTION; rayY++) {
            ...
            if(isInCircle) {
              ...
              const { top: topCoordinates, left: leftCoordinates } = rotationToTopAndLeftCoordinates(this.rx, this.ry)
        ...
        this.layer.set(
          Math.round(this.x - BALL_RADIUS) + _x,
          Math.round(this.y - BALL_RADIUS) + _y,
          { r, g, b, a }
        )
