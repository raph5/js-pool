...
import { Quaternion, rotationToQuaternion } from "../../utils/quaternion"
...
export default class Ball {
  ...
  updateRotation() {
    // apply rotation
    const quat = rotationToQuaternion( this.rotationBufferAngle, this.rotationBufferVx, this.rotationBufferVy, 0 )
    this.rotate(quat)
  ...
  forward(deltaTime) {
    ...
    if(this.vx === this.rotationBufferVx && -this.vy === this.rotationBufferVy) {
      ...
      this.rotationBufferAngle += Math.sqrt( deltaX**2 + deltaY**2 ) / BALL_RADIUS
    }
    else {
